...
def list_page(page: Page):
    ...
    page.padding = 10
    page.scroll = "adaptive"  # Enable scrolling for smaller screens
    ...
    def load_items():
        ...
        for item in items:
            ...
            row = ft.DataRow(cells=[
                ...
                ft.DataCell(ft.Row([edit_button, delete_button], spacing=10))  # Wrap buttons in a row for responsiveness
            ])
    ...
    data_table = ft.DataTable(
        columns=[
            ...
            ft.DataColumn(ft.Text("Actions")),  # Merge edit and delete under one column
        ],
        rows=[],
        expand=True,  # Make the table responsive
    )

    # Add buttons and data table inside a responsive layout
    create_button = ft.ElevatedButton("Create New Item", on_click=go_to_create)

    layout = ft.ResponsiveRow(
        controls=[
            ft.Column(
                controls=[create_button, data_table],
                expand=True,  # Make the column take full width
                alignment=ft.alignment.center,
                scroll="adaptive",  # Enable scrolling for small screens
            )
        ]
    )
